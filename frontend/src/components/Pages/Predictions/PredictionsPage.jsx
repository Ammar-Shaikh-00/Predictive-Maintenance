...
export default function PredictionsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [{ snapshot: snap }, coverage] = await Promise.all([
      ...
      if (!snap?.available && !(snap?.predictions || []).length) {
        setError("Keine Live-Bewertungen vorhanden.");
      } else {
        setError(null);
      }
      setSnapshot(snap);
    ...
    <div className="w-full max-w-full space-y-4 pb-10 text-slate-100">
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-xs text-slate-400">
            Vorhersagen und empfohlene Aktionen aus den verbundenen Maschinen- und
            KI-Daten.
          </p>
        </div>
      ...
      <section className="grid gap-3 lg:grid-cols-2">
        ...
        <article className="rounded-xl border border-white/10 bg-[#12161e] p-4">
          ...
          {!run ? (
            <p className="text-sm text-slate-500">Keine Live-Bewertung vorhanden.</p>
          ) : (
            <>
