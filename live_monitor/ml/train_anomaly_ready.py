"""Train Isolation Forest on READY windows for pre-production anomaly detection."""
...
def main() -> None:
    ...
    # trains when enough READY labeled windows exist

    df = pd.read_csv(input_path)
    df = _align_feature_columns(df)

    # step 2: keep only READY rows for training
    train_mask = df["predicted_state"] == "READY"
    train_df = df.loc[train_mask].copy()

    # step 3: feature columns used for normal READY profile
    feature_cols = FEATURE_COLUMNS

    # step 4: drop rows with missing features
    train_df = train_df.dropna(subset=feature_cols)
    if len(train_df) == 0:
        raise RuntimeError("No training rows after filtering for READY with complete features.")
    ...
    # step 7: train isolation forest — learns normal READY behavior from data
