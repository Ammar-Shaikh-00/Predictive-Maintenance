"""Train Isolation Forest on COOLING-state windows; temperature fall is the key signal."""
...
def main() -> None:
    ...
    # key signal: slope_temperature negative (falling)

    df = pd.read_csv(input_path)
    df = _align_feature_columns(df)

    # step 2: keep only COOLING rows — learn typical cool-down dynamics
    train_mask = df["predicted_state"] == "COOLING"
    train_df = df.loc[train_mask].copy()

    # step 3: cooling-focused features — matches state classifier feature set
    feature_cols = FEATURE_COLUMNS

    # step 4: drop rows with missing features
    train_df = train_df.dropna(subset=feature_cols)
    if len(train_df) == 0:
        raise RuntimeError("No training rows after filtering for COOLING with complete features.")
