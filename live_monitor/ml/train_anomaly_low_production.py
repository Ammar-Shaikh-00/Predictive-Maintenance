"""Train Isolation Forest on stable LOW_PRODUCTION windows for process anomaly detection."""
...
def main() -> None:
    ...
    # step 2: keep only stable LOW_PRODUCTION rows for training
    train_mask = df["predicted_state"] == "LOW_PRODUCTION"
    if "is_stable" in df.columns:
        train_mask = train_mask & (df["is_stable"] == True)  # noqa: E712
    train_df = df.loc[train_mask].copy()

    # step 3: feature columns used for normal LOW_PRODUCTION profile
    ...
    if len(train_df) == 0:
        raise RuntimeError(
            "No training rows after filtering for stable LOW_PRODUCTION with complete features."
        )
    ...
    # step 7: train isolation forest — learns normal LOW_PRODUCTION behavior from data
    ...
    # step 8: validate on training data
