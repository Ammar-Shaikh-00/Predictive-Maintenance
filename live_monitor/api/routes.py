...
def _state_classifier_loaded() -> bool:
    """True when state classifier is loaded in-process, else when .pkl files exist on disk."""
    try:
        import main as pipeline_main

        detector = getattr(pipeline_main, "detector", None)
        if detector is not None and getattr(detector, "classifier", None) is not None:
            return True
    except (ImportError, AttributeError):
        pass
    ...
    return os.path.isfile(classifier_path) and os.path.isfile(scaler_path)


def _drift_baseline_loaded() -> bool:
    try:
        import main as pipeline_main

        dd = getattr(pipeline_main, "drift_detector", None)
        if dd is None:
            return False
        return bool(getattr(dd, "baseline_stats", None))
    except (ImportError, AttributeError):
        return False


def _expected_sensor_channels() -> list[str]:
    """Mapped channels used by live_sensors capability probe."""
    return [
        config.FIELD_SCREW_SPEED,
        config.FIELD_LOAD,
        config.FIELD_PRESSURE,
        *list(config.FIELD_TEMPERATURE_ZONES),
    ]


@app.get("/health")
def health():
    """Capability scorecard probes read this endpoint (see Docs/capability_component_catalog.json)."""
    models_loaded = _ml_models_loaded()
    models_expected = list(MODEL_REGISTRY.keys())
    classifier_loaded = _state_classifier_loaded()
    drift_baseline_loaded = _drift_baseline_loaded()
    # Serving readiness only — never Accuracy %. Null until at least one anomaly model is loaded.
    if models_loaded and classifier_loaded:
        serving_readiness = round(len(models_loaded) / max(len(models_expected), 1) * 100)
    else:
        serving_readiness = None
    ...
    return {
        ...
        "value_source": "LIVE",
        # --- capability probes (AI/ML catalog) ---
        "ml_models_loaded": models_loaded,
        "models_expected": models_expected,
        "models_expected_count": len(models_expected),
        "classifier_loaded": classifier_loaded,
        "state_classifier_loaded": classifier_loaded,
        "drift_baseline_loaded": drift_baseline_loaded,
        "drift_status": "baseline_loaded" if drift_baseline_loaded else "no_baseline",
        "expected_ml_states": list(config.EVALUABLE_STATES),
        "expected_sensor_channels": _expected_sensor_channels(),
        "expected_sensor_channel_count": len(_expected_sensor_channels()),
        # Not Accuracy — serving health for prediction_readiness formula only.
        "prediction_readiness": serving_readiness,
        "prediction_readiness_kind": "serving_health_not_accuracy",
