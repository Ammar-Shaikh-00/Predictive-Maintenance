...
async def _ensure_extruder_machine_and_sensor(session: AsyncSession) -> Tuple[UUID, UUID]:
    ...
    machine = (await session.scalars(
        sql_select(Machine).where(Machine.name == "Extruder").limit(1)
    )).first()
    if machine is None:
...
async def get_current_dashboard_data(
    ...
    # Ensure the expected Extruder machine exists (bootstrap if missing)
    try:
        machine_id, _sensor_id = await _ensure_extruder_machine_and_sensor(session)
        machine = await session.get(Machine, machine_id)
    except Exception as e:
        logger.warning("Failed to ensure extruder machine for /dashboard/current: %s", e)
        machine = None

    if not machine:
        raise HTTPException(status_code=404, detail="Extruder machine not found (bootstrap failed)")
    
    # Query latest data from TimescaleDB for state calculation
    ...
    if tsdb_client.tsdb_configured():
        try:
            rows_tsdb = await tsdb_client.fetch_extruder_latest_from_tsdb(limit=1)
            if rows_tsdb:
                current_row = rows_tsdb[0]
                latest_timestamp = current_row.get("TrendDate")
        except Exception as e:
            logger.warning("TimescaleDB error in /dashboard/current: %s", e)
    
    # Get current machine state - compute from latest data if available
    state_service = MachineStateService(session)
    current_state = None
    
    # If we have latest sensor data, process it through the state detector
    if current_row and latest_timestamp:
        try:
            from app.services.machine_state_service import SensorReading
            
            # Create SensorReading from latest sensor data
            sensor_reading = SensorReading(
                ...
                motor_load= current_row.get("Motor_load"),
        ...
        # No sensor data available - use get_current_state (may return OFF if no readings)
        current_state = await state_service.get_current_state(str(machine.id))
    ...
    # current_row and latest_timestamp are from TimescaleDB above
    ...
    if current_row:
        ...
        if len(valid_temps) >= 2:
            ...
            metrics_response["Motor_load"] = {
                "current_value": as_float(current_row.get("Motor_load")),
                "baseline_mean": None,
                "green_band": None,
                "deviation": None,
                "severity": -1,
    ...
    # PRODUCTION state: Get full evaluation data (window from TimescaleDB)
    from datetime import datetime, timedelta
    from app.services import tsdb_client
    ...
    if tsdb_client.tsdb_configured():
        try:
            limit = max(200, window_minutes * 6)
            all_rows = await tsdb_client.fetch_extruder_latest_from_tsdb(limit=limit)
            cutoff = datetime.utcnow() - timedelta(minutes=window_minutes)
            for r in all_rows:
                td = r.get("TrendDate")
                if td:
                    try:
                        ts = datetime.fromisoformat(str(td).replace("Z", "+00:00")) if isinstance(td, str) else td
                    ...
                    if ts >= cutoff:
                        rows.append(r)
            rows.sort(key=lambda x: (x.get("TrendDate") or ""))
        except Exception as e:
            logger.warning("TimescaleDB error in /current PRODUCTION branch: %s", e)
    ...
    all_metric_keys = sensor_keys + ["Temp_Avg", "Temp_Spread"] + ["Motor_load"]
    ...
    for key in all_metric_keys:
        ...
        # Get stability state for this sensor (must be set before explanation uses it)
    ...
    all_metric_keys_for_severity = sensor_keys + ["Temp_Avg", "Temp_Spread"] + ["Motor_load"]
...
async def get_extruder_latest_rows(
    ...
    """Return latest extruder sensor rows from TimescaleDB (Tab_Actual)."""
    ...
    if not tsdb_client.tsdb_configured():
        _extruder_last_error = "TimescaleDB not configured"
        _extruder_last_error_at = datetime.utcnow()
        raise HTTPException(
            status_code=503,
            detail="Extruder data source is TimescaleDB. Set TSDB_HOST, TSDB_USER, TSDB_PASSWORD (and optionally TSDB_DATABASE, TSDB_PORT).",
        )
    try:
        rows = await tsdb_client.fetch_extruder_latest_from_tsdb(limit=limit)
        _extruder_last_success_at = datetime.utcnow()
        _extruder_last_error = None
        _extruder_last_error_at = None
        return {"rows": rows}
    except Exception as e:
        logger.exception("TimescaleDB extruder latest failed")
        _extruder_last_error = f"{type(e).__name__}: {e}"
        _extruder_last_error_at = datetime.utcnow()
        raise HTTPException(status_code=502, detail="Failed to read extruder data from TimescaleDB")


@router.get("/extruder-latest-values")
async def get_extruder_latest_row_all_columns(
    # current_user: User = Depends(require_viewer),
    session: AsyncSession = Depends(get_session),
):
    """Return latest extruder sensor rows from TimescaleDB (Tab_Actual)."""
    ...
    try:
        rows = await tsdb_client.fetch_extruder_latest_all_columns_from_tsdb()
        _extruder_last_success_at = datetime.utcnow()
        _extruder_last_error = None
        _extruder_last_error_at = None
        return {"rows": rows}
...
@router.get("/extruder/timeRangeData")
async def get_extruder_time_range_data(
    fromDate: datetime = Query(..., description="Start datetime"),
    toDate: datetime = Query(..., description="End datetime"),
    limit: int = Query(500000, ge=1, le=500000),
):
    """
    Return extruder sensor data between fromDate and toDate.
    """
    ...
    if not tsdb_client.tsdb_configured():
        raise HTTPException(
            status_code=503,
            detail="Extruder data source is TimescaleDB. Set TSDB_HOST, TSDB_USER, TSDB_PASSWORD.",
        )
    ...
    try:
        rows = await tsdb_client.fetch_extruder_history_by_date_range(
            from_date=fromDate,
            to_date=toDate,
            limit=limit,
        )
        ...
        logger.exception("TimescaleDB extruder date range fetch failed")
        raise HTTPException(
            status_code=502,
            detail="Failed to read extruder history from TimescaleDB",
        )
...
async def get_extruder_history_from_sensor_data(
    ...
    hours: int | None = Query(None, ge=1, le=48, description="If set, return rows from the last N hours (e.g. 24 for full 1d chart)"),
    limit: int = Query(5000, ge=1, le=500000),
    # session: AsyncSession = Depends(get_session),
):
    """
    Return historical extruder sensor values from TimescaleDB (Tab_Actual).
    Use hours=24&limit=500000 for full last 24h coverage (1d chart).
    """
    from app.services import tsdb_client
    if not tsdb_client.tsdb_configured():
        raise HTTPException(
    ...
    try:
        rows = await tsdb_client.fetch_extruder_history_from_tsdb(days=days, limit=limit, hours=hours)
        return {"rows": rows}
    except Exception as e:
        logger.exception("TimescaleDB extruder history failed")
        raise HTTPException(status_code=502, detail="Failed to read extruder history from TimescaleDB")


@router.get("/extruder/history/daily")
async def get_extruder_history_daily(
    # current_user: User = Depends(require_viewer),
    days: int = Query(365, ge=1, le=730, description="Number of days of history (one row per day)"),
    session: AsyncSession = Depends(get_session),
):
    """
    Return one row per calendar day with daily max per sensor from TimescaleDB.
    Use for "All" chart so all dates in range are returned without row limit (e.g. 365 rows for 365 days).
    Shape: TrendDate (noon of day), ScrewSpeed_rpm, Pressure_bar, Temp_Zone1_C..4.
    """
    from app.services import tsdb_client
    if not tsdb_client.tsdb_configured():
        raise HTTPException(
    ...
    try:
        rows = await tsdb_client.fetch_extruder_history_daily_from_tsdb(days=days)
        return {"rows": rows}
    except Exception as e:
        logger.exception("TimescaleDB extruder history daily failed")
        raise HTTPException(status_code=502, detail="Failed to read extruder history (daily) from TimescaleDB")
...
async def get_extruder_history_diagnostic(
    ...
    History is read from TimescaleDB (Tab_Actual). Returns TSDB connection status and next steps.
    """
    from app.services import tsdb_client

    timescaledb_configured = tsdb_client.tsdb_configured()
    timescaledb_connected = False
    timescaledb_message = ""
    if timescaledb_configured:
        timescaledb_connected, timescaledb_message = await tsdb_client.check_tsdb_connection()

    issues = []
    if not timescaledb_configured:
        issues.append("TimescaleDB not configured — set TSDB_HOST, TSDB_USER, TSDB_PASSWORD (and optionally TSDB_DATABASE, TSDB_PORT).")
    elif not timescaledb_connected:
        issues.append(f"TimescaleDB connection failed: {timescaledb_message}")
    if not issues:
        issues.append("TimescaleDB is configured and connected. If history is empty, ensure Tab_Actual has data for the requested time range.")

    return {
        "data_source": "timescaledb",
        "timescaledb_configured": timescaledb_configured,
        "timescaledb_connected": timescaledb_connected,
        "timescaledb_message": timescaledb_message,
        "issues": issues,
        "next_steps": [
            "Ensure TSDB_HOST, TSDB_USER, TSDB_PASSWORD are set in backend environment.",
            "For Docker: use service name for host (e.g. TSDB_HOST=timescaledb) and ensure the timescaledb container is running.",
            "Verify Tab_Actual exists and has rows: SELECT COUNT(*) FROM \"Tab_Actual\";",
        ],
    }
...
async def get_extruder_status(
    ...
    """Extruder connection status. Sensor data is read from TimescaleDB only."""
    ...
    # Get extruder machine by name for profile/baseline info
    ...
    return {
        "configured": timescaledb_configured,
        "timescaledb_configured": timescaledb_configured,
        "timescaledb_connected": timescaledb_connected,
        "timescaledb_message": timescaledb_message,
        "extruder_data_source": "tsdb",
        ...
        "last_error": None if timescaledb_connected else _extruder_last_error,
    }
...
async def get_extruder_derived_kpis(
    ...
    # Read window from TimescaleDB (main sensor data source).
    from app.services import tsdb_client
    rows: list = []
    if not tsdb_client.tsdb_configured():
        _extruder_last_error = "TimescaleDB not configured"
    ...
    try:
        ...
        for r in all_rows:
            ...
            if td:
                try:
                    ts = datetime.fromisoformat(str(td).replace("Z", "+00:00")) if isinstance(td, str) else td
                    if hasattr(ts, "timestamp"):
                        ts = datetime.utcfromtimestamp(ts.timestamp()) if hasattr(ts, "timestamp") else ts
                except Exception:
                    continue
                if ts >= cutoff:
                    rows.append(r)
        ...
        _extruder_last_error_at = datetime.utcnow()
        _extruder_last_error = f"TimescaleDB error: {e}"
        logger.warning("TimescaleDB error in /extruder/derived: %s", e)
        rows = []
    ...
    def _parse_trend_date(td):
        """Parse TrendDate (string or datetime) to datetime for comparison."""
        if td is None:
            return None
        if isinstance(td, datetime):
            return td
        try:
            s = str(td).strip().replace("Z", "+00:00")
    ...
    for metric_key, metric_label in stability_metrics.items():
        ...
        if metric_key == "Temp_Avg":
            ...
            current_vals = [
                r.get("Temp_Avg")
                for r in rows
                if r.get("Temp_Avg") is not None and _parse_trend_date(r.get("TrendDate")) is not None and _parse_trend_date(r.get("TrendDate")) >= ten_min_ago
            ]
        else:
            current_vals = [
                ...
                and _parse_trend_date(r.get("TrendDate")) is not None
                and _parse_trend_date(r.get("TrendDate")) >= ten_min_ago
            ]
