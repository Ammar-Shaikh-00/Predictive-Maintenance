...
from app.schemas.default_machine_state import DefaultMachineStateCreate, DefaultMachineStateOut
...
@router.post(
    ...
    response_model=DefaultMachineStateOut,
    status_code=status.HTTP_201_CREATED,
)
async def create_machine_state(
    machine_state_in: DefaultMachineStateCreate,
    db: AsyncSession = Depends(get_session),
):
    machine_state = DefaultMachineState(**machine_state_in.model_dump())
    db.add(machine_state)
    await db.commit()
    await db.refresh(machine_state)
    return machine_state


@router.get("/default-machine-states", response_model=list[DefaultMachineStateOut])
async def get_machine_states(db: AsyncSession = Depends(get_session)):
    result = await db.execute(select(DefaultMachineState))
...
async def get_all_current_states(
    ...
    try:
        ...
        if machine:
            # Get latest sensor row from TimescaleDB (or MSSQL if TSDB not configured)
            current_row = {}
            ...
            if tsdb_client.tsdb_configured():
                try:
                    ...
                    logger.warning("TimescaleDB error in /states/current: %s", e)
            else:
                # Fallback: query MSSQL for latest data (when TSDB not configured)
            ...
            if current_row and latest_timestamp:
                ...
                # No sensor data available - use get_all_current_states (may return OFF if no readings)
                states = await state_service.get_all_current_states()
        else:
